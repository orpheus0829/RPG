...
#define NORMAL_MAP_FILE "../tex2/Tex_123_N.png"
...
#define SMOOTHNESS_MAP_FILE "../tex2/Tex_123_M2.png"
...
#define METALNESS_MAP_FILE "../tex2/Tex_123_M1.png"
...
#define SPECULAR_MAP_FILE "../tex2/Tex_123_M1.png"
...
float3 GetAlbedo(float2 coord)
{
		...
		#ifdef ALBEDO_SINGLE_CHANNEL
		albedoColor = albedoColor.rrr;
		#endif
	#elif ALBEDO_MAP_FROM == 4
...
float3 GetMainNormal(float3 N, float3 P, float2 coord)
{
	...
	#else
		float2 ba = tex2D(NormalMapSamp, coord * normalMapLoopNum).gr;
        // ba.g = 1 - ba.g;
		float3 tangentNormal = float3(ba, 1.0) * 2 - 1;
	#endif
...
float3 GetEmissiveColor(float2 coord)
{
		...
		float3 emissiveColor = emissiveTexCol.b;
	...
	in float4 worldPos : TEXCOORD3)
{
	...
	coord0 = ParallaxOcclusionMap(ParallaxMapSamp, coord0 * parallaxMapLoopNum, viewdir, 15, parallaxMapScale);
	#if PARALLAX_MAP_UV_FLIP
		coord0.y = 1 - coord0.y;
	#endif
#endif
	...
	material.emissive = GetEmissiveColor(coord0) * material.albedo;
