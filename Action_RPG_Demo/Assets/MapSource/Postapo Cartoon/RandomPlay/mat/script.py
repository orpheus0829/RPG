...
for filename in fs:
    ...
    if len(a) == 2:
        ...
        m = z[0] + "_M" + "." + z[1]
        if n in fs:
            c = c.replace('#define NORMAL_MAP_FROM 0', "#define NORMAL_MAP_FROM 1")
            c = c.replace("#define NORMAL_MAP_FILE \"normal.png\"", "#define NORMAL_MAP_FILE \"../tex2/{}\"".format(n))
        if m in fs:
            c = c.replace('#define METALNESS_MAP_FROM 0', "#define METALNESS_MAP_FROM 1")
            c = c.replace("#define METALNESS_MAP_FILE \"metalness.png\"", "#define METALNESS_MAP_FILE \"../tex2/{}\"".format(m))
            c = c.replace('#define SMOOTHNESS_MAP_FROM 0', "#define SMOOTHNESS_MAP_FROM 1")
            c = c.replace('#define SMOOTHNESS_MAP_SWIZZLE 0', "#define SMOOTHNESS_MAP_SWIZZLE 1")
            c = c.replace("#define SMOOTHNESS_MAP_FILE \"smoothness.png\"", "#define SMOOTHNESS_MAP_FILE \"../tex2/{}\"".format(m))
            c = c.replace("#define EMISSIVE_MAP_FILE \"emissive.png\"", "#define EMISSIVE_MAP_FILE \"../tex2/{}\"".format(m))
        
        output = output_folder + "mat_" + a[1].replace(".png", ".fx")
