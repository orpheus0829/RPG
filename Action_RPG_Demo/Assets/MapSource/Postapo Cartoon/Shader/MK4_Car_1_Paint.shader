...
/////// Diffuse:
                ...
                float node_9100 = saturate(pow(_AORMask1GMask2B_var.r,(_AO*2.0+0.0)));
                indirectDiffuse *= node_9100; // Diffuse AO
/// Final Color:
                outDiffuse = half4( diffuseColor, node_9100 );
                outSpecSmoothness = half4( specularColor, gloss );
                outNormal = half4( normalDirection * 0.5 + 0.5, 1 );
                outEmission = half4(0,0,0,1);
                outEmission.rgb += indirectSpecular * node_9100;
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _AORMask1GMask2B_var = tex2D(_AORMask1GMask2B,TRANSFORM_TEX(i.uv0, _AORMask1GMask2B));
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float3 diffColor = (lerp(_ColorMain.rgb,_Color1.rgb,_AORMask1GMask2B_var.g)*_MainTex_var.rgb);
                float specularMonochrome;
                float3 specColor;
                float4 _MetallicGloss_var = tex2D(_MetallicGloss,TRANSFORM_TEX(i.uv0, _MetallicGloss));
